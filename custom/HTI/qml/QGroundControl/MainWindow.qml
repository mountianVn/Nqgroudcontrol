...
ApplicationWindow {
    ...
    function showSettingsTool(settingsPage = "") {
        showTool(qsTr("Application Settings"), "qrc:/Custom/qml/QGroundControl/Controls/AppSettings.qml", "/res/QGCLogoWhite")
        if (settingsPage !== "") {
    ...
    // HTI Custom ToolDrawer - Responsive size for settings panel
    Rectangle {
        id:             toolDrawer
        objectName:     "mainView_toolDrawer"
        anchors.centerIn: parent
        // Responsive dimensions: 80% of FlyView, with min/max constraints
        width:          Math.max(900, Math.min(parent.width * 0.8, 1200))
        height:         Math.max(650, Math.min(parent.height * 0.8, 800))
        visible:        false
        color:          qgcPal.window
        ...
        Rectangle {
            ...
            anchors.right:  parent.right
            anchors.top:    parent.top
            height:         0  // HTI: Hidden, Settings header is in AppSettings.qml
            visible:        false
        }
